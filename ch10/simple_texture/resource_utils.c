...
/* Create a sampler for reading textures */
void create_sampler(Context* ctx) {

  /* Check if the device supports anisotropy */
  VkPhysicalDeviceFeatures features;
  vkGetPhysicalDeviceFeatures(ctx->phys_device, &features);
  ...
  ck(vkCreateSampler(ctx->device, &sampler_info, NULL, &ctx->sampler));
}
