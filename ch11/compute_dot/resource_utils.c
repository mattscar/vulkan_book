...
/* Transfer data to image on device */
void transfer_to_device_image(Context* ctx, void* data,
  ...
  vkCmdCopyBufferToImage(temp_command_buffer,
    temp_buffer, *device_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 
    1, &buffer_image_copy);
...
void read_from_device_buffer(Context* ctx, VkDeviceSize src_offset,
  ...
  /* Structures needed to copy device buffer data */
  VkBuffer dst_buffer;
  ...
  /* Create buffer to hold ouptut data */
  create_buffer(ctx, VK_BUFFER_USAGE_TRANSFER_DST_BIT,
    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, data_size, NULL,
    &dst_memory, &dst_buffer);
  ...
  vkCmdCopyBuffer(temp_command_buffer, *src_buffer,
     dst_buffer, 1, &buffer_copy);
  ...
  ck(vkQueueSubmit(ctx->queues[TRANSFER_QTYPE], 1, &submit_info, NULL));
  ...
  /* Map a memory address to the resource */
  void *mem_ptr;
  ck(vkMapMemory(ctx->device, dst_memory, 0, data_size, 0, &mem_ptr));

  /* Copy data from the memory address */
  memcpy(data, mem_ptr, data_size);
  ...
  ck(vkFlushMappedMemoryRanges(ctx->device, 1, &map_range));

  /* Remove the mapping and deallocate resources */
  vkUnmapMemory(ctx->device, dst_memory);
  destroy_buffer(ctx, &dst_buffer, &dst_memory);
}
