...
void createSync(Context& ctx) {
  ...
  auto acqRes = ctx.device.createSemaphore(semaphoreInfo);
  ck(acqRes.result);
  ctx.acquireCompleteSemaphore = acqRes.value;
  
  auto commRes = ctx.device.createSemaphore(semaphoreInfo);
  ck(commRes.result);
  ctx.commandCompleteSemaphore = commRes.value;

  // Create fence
  vk::FenceCreateInfo fenceInfo = vk::FenceCreateInfo();
  auto fenceRes = ctx.device.createFence(fenceInfo);
  ctx.fence = fenceRes.value;  
