...
void createFramebuffers(Context& ctx) {
  ...
  for(uint32_t i=0; i<ctx.numImages; ++i) {
    ...
    resource_utils::createImageView(ctx, ctx.imageFormat.format, 
    ...
    vk::FramebufferCreateInfo framebufferCreateInfo = vk::FramebufferCreateInfo()
      ...
      .setLayers(1);
    auto res = ctx.device.createFramebuffer(framebufferCreateInfo);
    ck(res.result);
    ctx.framebuffers.push_back(res.value);
  }
}
