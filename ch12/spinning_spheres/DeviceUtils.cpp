...
void createDevice(Context& ctx,
  ...
  // Create an instance
  auto res = vk::createInstance(instInfo);
  ck(res.result);
  ctx.instance = res.value;
  ...
  auto physRes = ctx.instance.enumeratePhysicalDevices();
  ck(physRes.result);
  devices = physRes.value;
  ...
  auto devRes = ctx.physDevice.createDevice(deviceInfo);
  ck(devRes.result);
  ctx.device = devRes.value;
