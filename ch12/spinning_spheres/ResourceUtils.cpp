...
void allocateMemory(Context& ctx, vk::MemoryRequirements memReqs,
  ...
  auto res = ctx.device.allocateMemory(memInfo);
  ck(res.result);
  deviceMem = res.value;
}
...
void initResource(vk::Device dev, vk::MemoryPropertyFlags type,
  ...
  auto res = dev.mapMemory(deviceMem, 0, size);
  ck(res.result);
  void *memPtr = res.value;
  ...
  if(!(type & vk::MemoryPropertyFlagBits::eHostCoherent)) {
    ...
    ck(dev.flushMappedMemoryRanges(1, &mapRange));
  }
...
void createBuffer(Context& ctx, vk::BufferUsageFlags usage,
  ...
  auto res = ctx.device.createBuffer(bufferInfo);
  ck(res.result);
  buffer = res.value;
  ...
  ck(ctx.device.bindBufferMemory(buffer, deviceMem, 0));
}
...
void transferToDeviceBuffer(Context& ctx, void* data,
  ...
  tempBeginInfo =
    command_utils::createCommandBuffers(ctx, tempCommandBuffers,
      tempCommandPool, 1, TRANSFER_QTYPE,
      vk::CommandBufferUsageFlagBits::eOneTimeSubmit);
      
  // Start recording commands
  ck(tempCommandBuffers[0].begin(tempBeginInfo));
  ...
  ck(ctx.queues[TRANSFER_QTYPE].submit(tempInfo, nullptr));
...
void createImageView(Context& ctx, vk::Format format,
  ...
  auto res = ctx.device.createImageView(imageViewInfo);
  ck(res.result);
  imageView = res.value;
}
